# Stripe Payment Site — dynamic booking amount
...
- Начальная сумма может передаваться через `?amount=`; допустимый диапазон — $0.50–$2,000.00.
...
- Добавлен защищённый `/api/charge-followup` для последующего платежа, сумма которого задаётся `FOLLOWUP_AMOUNT_USD`.
...
The public payment page accepts an amount in the URL, for example `/?amount=3`. The backend validates the amount (minimum $0.50, maximum $2,000.00) and creates the Stripe Checkout Session for that amount. If the parameter is omitted, `INITIAL_AMOUNT_USD` is used as the default. The follow-up amount remains controlled by `FOLLOWUP_AMOUNT_USD` on the server.

For a custom domain on Vercel, use the custom domain in the customer-facing payment link. Success and cancel redirects use the hostname of the request automatically.
